...
// Modified this function to accept GET, POST, PUT, DELETE and CONNECT Requests
int ParsedRequest_parse(struct ParsedRequest *parse, const char *buf, int buflen) {
    ...
    // Allow GET, POST, CONNECT, PUT, and DELETE methods
    if (strcmp(parse->method, "GET") && strcmp(parse->method, "POST") && strcmp(parse->method, "CONNECT") &&
        strcmp(parse->method, "PUT") && strcmp(parse->method, "DELETE")) {
        debug("invalid request line, method not 'GET', 'POST', 'CONNECT', 'PUT', or 'DELETE': %s\n", parse->method);
        free(tmp_buf);
        free(parse->buf);
        parse->buf = NULL;
        return -1;
